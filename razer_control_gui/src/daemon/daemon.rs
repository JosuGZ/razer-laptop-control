...
fn start_battery_monitor_task() -> JoinHandle<()> {
    thread::spawn(move || {
        ...
        loop {
            dbus_system
                .process(time::Duration::from_millis(1000))
                .unwrap();
        }
    })
}
...
pub fn process_client_request(cmd: comms::DaemonCommand) -> Option<comms::DaemonResponse> {
    ...
    match cmd {
        ...
        comms::DaemonCommand::SetFanSpeed { ac, rpm } => Some(comms::DaemonResponse::SetFanSpeed {
        ...
        comms::DaemonCommand::GetBrightness { ac } => Some(comms::DaemonResponse::GetBrightness {
        ...
        comms::DaemonCommand::SetEnableLightControl { enable } => {
            Some(comms::DaemonResponse::SetEnableLightControl {
                result: d.set_light_control(enable),
            })
        }
        comms::DaemonCommand::GetEnableLightControl => {
            Some(comms::DaemonResponse::GetEnableLightControl {
                enabled: d.get_light_control(),
            })
        }
